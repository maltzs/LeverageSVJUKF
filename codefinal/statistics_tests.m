...
% STATISTICS_TESTS Runs various statistics on the residual series from the
% stochastic-volatility joint unscented Kalman filter.
%   Inputs:
%   - z_hat: Vector of the residual series.
%   - figs: Logical value whether figures and tables should be produced.
%
%   Outputs:
%   - stats: A struct with the statistics. Has the following fields:
%       - statistics: A struct with sample statistics. Has the following
%       fields:
%           - mean: Mean of z_hat.
...
%           - zsquared: Sample autocorrelation of z_hat squared.
%
...
%   of Science and Art (2022).
    ...
    [~,stats.tests.plbq] = lbqtest(z_hat.^2);
    [~,stats.tests.parch] = archtest(z_hat);
    ...
    if figs
        table(["Mean"; "Variance"; "Skewness"; "Kurtosis"], ...
            ...
            stats.statistics.skewness; stats.statistics.kurtosis], ...
