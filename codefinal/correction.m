function [a_hatcorr, P_corr] = correction(M, mu, P, sigma_nu, S_nu, y)
...
%   - mu: Current state prediction vector. Should be of length M+2 where
%   the last entry is mu_nu.
%   - P: Current prediction covariance. Must be (M+1) x (M+1) and positive
%   definite.
...
%
%   Outputs:
%   - a_hatcorr: Vector with the corrected state. Is of size M+1.
...
%   of Science and Art (2022).
    ...
    % Create sigma-points which take the distribution of nu into account.
    [w, chi] = create_sigma_points(M, mu, P, sigma_nu, S_nu);
    
    chi_a = chi(1:end-1,:);    % augmented state sigma point components
    chi_y = chi(end,:);        % measurement sigma point component
    y_hat = chi_y * w;
...
% CREATE_SIGMA_POINTS Creates sigma points that take the distribution of nu
% into account.
%   Inputs:
%   - M: Number of estimated parameters.
%   - mu: Current state estimate vector. Should be of length M+2 where
%   the last entry is mu_nu.
%   - C: Current prediction covariance. Must be (M+1) x (M+1) and positive
%   definite.
%   - sigma_nu: Standard deviation of nu.
%   - S_nu: Skewness of nu.
%
