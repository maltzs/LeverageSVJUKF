function leverage_SVJUKF_sim(N_sim, T, jumps, N_particles, theta, M, ...
    width, P_corr, Q_noise, sp, figs, avg, ukf, pf)
% LEVERAGE_SVJUKF_SIM Wrapper for the leverage Stochastic-volatility joint
% unscented Kalman filter (SV-JUKF) model. Runs a certain number of
% simulations.
%   Inputs:
...
%   jumps.
%   - N_particles: Number of particles to use in particle filter. For use
%   only when pf is true.
%   - theta: Matrix of true theta values. Each column represents one set of
%   theta values. The number of columns is the number of sets of theta
%   values that can be jumped to. theta(2,i) must be between 0 and 1 and
%   theta(3,i) must be greater than 0 for all columns i.
...
%   - Q_noise: Vector of parameter estimate variances. Must have length M.
...
%   - figs: Logical value whether figures and tables should be produced for
%   every simulation or not.
%   - avg: Logical value whether to average and plot the absolute value
%   sample autocorrelations of the simulations.
%   - ukf: Logical value whether to also run a SV-UKF to compare against
%   the JUKF or not.
%   - pf: Logical value whether to also run a particle filter to compare
%   against the SV-JUKF or not.
%
%   Samuel Maltz, Master's thesis at The Cooper Union for the Advancement
%   of Science and Art (2022).

    % Threshold generalized asymmetric stochastic volatility model.
    f = @(epsilon, alpha, gamma_1, gamma_2) alpha*((epsilon < 0)-0.5) ...
        + gamma_1*epsilon+gamma_2*(abs(epsilon)-sqrt(2/pi));

    % Used to compare parameter estimates and elapsed time for the particle
    % filter and SV-JUKF.
    if pf
    ...
    while n <= N_sim
        ...
        if sp
            ...
            % calculated used the closing prices which were obtained from
            % Yahoo Finance.
            load("spall.mat","u");
            T = length(u);
            y = log(abs(u));    % log-abs-log returns
        else
            % Initial theta estimates. phi_hat is ensured to be between 0
            % and 1.
        ...
        if ~sp
            for t = 1:T
                ...
                x(t+1) = theta(1,k)+theta(2,k)*x(t) ...
                    +f(epsilon, theta(3,k), theta(4,k), theta(5,k)) ...
                    +randn*theta(6,k);
        
                theta_plot(:,t) = [theta(1,k) theta(2,k) theta(3,k) ...
                    theta(4,k) theta(5,k)];
            end
        end
        ...
        try
            % Runs leverage SV-JUKF. On off chance of model breakdown,
            % redoes simulation (see catch).
            tic;
            ...
            if ukf
                % Runs leverage SV-UKF. On off chance of model breakdown,
                % redoes simulation (see catch).
            ...
            if pf
                % Runs leverage SV-UKF with final theta estimates from
                % SV-JUKF to compute residual series to compare with
                % particle filter. On off chance of model breakdown, redoes
                % simulation (see catch).
                ...
                % compare with SV-JUKF. On off chance of model breakdown,
                % redoes simulation (see catch).
                tic;
                theta_pf(n,:) = leverage_SVPF(T, N_particles, M, u, ...
                    theta_hat);
                time_pf(n) = toc;
                [~, z_hat_pf] = particle_filter(u, T, N_particles, ...
                    theta_pf(n,:), theta_hat(6), false);
    ...
    % Averages and plots the absolute value sample autocorrelations of the
    % simulations.
    if avg
        if ~pf
        ...
        if pf
            ...
            table(["mu(1-phi)"; "phi"; "alpha"; "gamma_1"; "gamma_2"], ...
                mean(theta_jukf(1:M),1)',std(theta_jukf(1:M),0,1)', ...
                mean(theta_pf(1:M),1)',std(theta_pf(1:M),0,1)', ...
                'VariableNames',["Parameter","SV-JUKF avg", ...
                "SV-JUKF std","SV-PF avg","SV-PF std"])

            table("Time",mean(time_jukf,1),std(time_jukf,0,1), ...
                mean(time_pf,1),std(time_pf,0,1),'VariableNames', ...
                ["Quantity","SV-JUKF avg","SV-JUKF std","SV-PF avg", ...
                "SV-PF std"])
        end
    end   
end