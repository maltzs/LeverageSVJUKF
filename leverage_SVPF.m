...
%
%   Throws:
%   - MATLAB:nonaninf: If particle filter breaks down and returns
%   NaN or Inf.
%
...
%   Advancement of Science and Art (May 2022)
    ...
    theta_0(2) = inv_sig(theta_0(2));

    % Initializes optimization stopping condition variables.
    patience = 0;
    prev = theta_0;
    
    % First wrapper for particle filter function. Allows for
    % additional inputs. Outputs the likelihood value from
    % particle filter.
    pf_wrapper = @(theta) pf(y, T, N_particles, theta, ...
        sigma_eta_hat);

    % Options for optimizer to allow for early stopping.
    options = optimset('OutputFcn',@outfun);
    
    % Downhill simplex method of (Nelder, Mead, 1965), no
    % requirements on smoothness but is unconstrained. On off
    % chance of model breakdown, redoes simulation (see catch in
    % leverage_SVJUKF_sim).
...
%   - optimValues: Struct with information about optimization
%   process. Has at least the following fields:
...
%   Advancement of Science and Art (May 2022)
    ...
    % Stops optimization if past iteration 75 and 10 consecutive
    % iterations with change in x less than 1e-3.
    stop = false;
    if norm(x-prev) < 1e-3
        patience = patience + 1;
